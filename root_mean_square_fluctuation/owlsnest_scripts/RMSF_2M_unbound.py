...
STRIDE = 210
STOP = 100000000
ref = md.load('../../gro_files/p16925-r39-c0.gro')         # ref
ref_ind = ref.topology.select("protein and name CA")             # residue 1
rmsf = dict()
for i in range(10000):
    try:
        t = md.load('../RUN38/curated_RUN38_p16925/RUN38CLONE%d.xtc'%i,top='../../gro_files/p16925-r39-c0.gro')
...
for i in range(0,700):
    M2_r38_res[i] = [item[i] for item in rmsf_list]
np.save("M2_r38_res", M2_r38_res)
rmsf_avg_grp_1 = []
for j in range(700):
    res_list = M2_r38_res[j]
...
for j in range(700):
    ...
    for i in range(1000):  # boot strapping
        ...
        np.save('./individual_means/M2_r38_res%d' %j, group1_avg)
#----------------------------------------------------------------------------------
START = 200
STRIDE = 210
STOP = 100000000
...
for i in range(10000):
    try:
        t = md.load('../RUN39/curated_RUN39_p16925/RUN39CLONE%d.xtc'%i,top='../../gro_files/p16925-r39-c0.gro')
        t = t[START:STOP:STRIDE]
        atom_indices1=t.topology.select("protein and name CA")
        t.superpose(ref, atom_indices=atom_indices1, ref_atom_indices=ref_ind)
        avg_xyz = np.mean(t.xyz[:, atom_indices1, :], axis=0)
...
M2_r39_res = dict()
for i in range(0,700):
    M2_r39_res[i] = [item[i] for item in rmsf_list]
np.save("M2_r39_res", M2_r39_res)
rmsf_avg_grp_1 = []
for j in range(700):
    res_list = M2_r39_res[j]
    rmsf_avg_rmsf = { i : res_list[i] for i in range(0, len(res_list) ) }
    rmsf_avg_grp_1.append(rmsf_avg_rmsf)
group1_avg = []
for j in range(700):
    ...
    for i in range(1000):  # boot strapping
        ...
        group1_avg.append(avg1)
        group1_avg.sort(reverse=False)
        np.save('./individual_means/M2_r39_res%d' %j, group1_avg)
